# Cron-Systemd

# 📋 ОТЧЕТ: Сравнение Cron и Systemd для планирования задач в Linux

## 📌 Информация о выполнении
- **Студент:** [ФИО]
- **Группа:** [Номер группы]
- **Дата выполнения:** 13.12.2025
- **Предмет:** Администрирование Linux

---

## 🎯 Цель работы
Сравнить два метода планирования периодических задач в Linux: традиционный **Cron** и современный **Systemd Timer**. Реализовать один и тот же скрипт обоими способами.

---

## 📁 Структура проекта
```
/home/david44/
├── cron_scripts/
│   └── send_request.sh      # Bash-скрипт для отправки HTTP-запросов
├── cron_requests.log        # Лог-файл выполнения скрипта через Cron
├── monitoring-stack/        # Дополнительные материалы
└── ansible-managed-host/    # Дополнительные материалы
```

---

## 📝 Часть 1: Реализация через Cron

### 1.1 Созданный bash-скрипт
**Файл:** `~/cron_scripts/send_request.sh`

```bash
#!/bin/bash
TARGET_URL="https://api.example.com/webhook"
curl -s -w "\nHTTP Status: %{http_code}\n" "$TARGET_URL" >> /home/david44/cron_requests.log 2>&1
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Request sent to $TARGET_URL" >> /home/david44/cron_requests.log
exit 0
```

*(Скриншот 1: Содержимое скрипта send_request.sh)*
[ВСТАВЬТЕ СКРИНШОТ 1 ЗДЕСЬ]

---

### 1.2 Настройка расписания в Crontab
**Команда для просмотра:** `crontab -l`

```
*/5 * * * * /home/david44/cron_scripts/send_request.sh
```

**Объяснение формата:**
```
*/5  *    *    *    *    /home/david44/cron_scripts/send_request.sh
 │    │    │    │    │      │
 │    │    │    │    │      └── Команда для выполнения
 │    │    │    │    └───────── День недели (0-7, 0=воскресенье)
 │    │    │    └────────────── Месяц (1-12)
 │    │    └─────────────────── День месяца (1-31)
 │    └──────────────────────── Час (0-23)
 └───────────────────────────── Минута (0-59) → каждые 5 минут
```

*(Скриншот 2: Вывод команды crontab -l)*
[ВСТАВЬТЕ СКРИНШОТ 2 ЗДЕСЬ]

---

### 1.3 Проверка работы Cron
**Лог-файл выполнения:** `~/cron_requests.log`

```bash
# Команда для просмотра логов
tail -10 ~/cron_requests.log
```

**Пример вывода:**
```
[2025-12-10 12:00:02] Request sent to https://api.example.com/webhook
HTTP Status: 000
[2025-12-10 12:05:01] Request sent to https://api.example.com/webhook
HTTP Status: 000
[2025-12-13 00:05:03] Request sent to https://api.example.com/webhook
HTTP Status: 000
```

*(Скриншот 3: Содержимое лог-файла)*
[ВСТАВЬТЕ СКРИНШОТ 3 ЗДЕСЬ]

---

### 1.4 Статус Cron демона
```bash
sudo systemctl status cron --no-pager | head -10
```

*(Скриншот 4: Статус службы cron)*
[ВСТАВЬТЕ СКРИНШОТ 4 ЗДЕСЬ]

---

## ⚡ Часть 2: Реализация через Systemd Timer

### 2.1 Systemd Service файл
**Файл:** `/etc/systemd/system/webcheck.service`

```ini
[Unit]
Description=Web Check Service via Systemd
After=network.target
Wants=network.target

[Service]
Type=oneshot
User=david44
ExecStart=/home/david44/cron_scripts/send_request.sh
StandardOutput=journal
StandardError=journal

[Install]
WantedBy=multi-user.target
```

*(Скриншот 5: Содержимое webcheck.service)*
[ВСТАВЬТЕ СКРИНШОТ 5 ЗДЕСЬ]

---

### 2.2 Systemd Timer файл
**Файл:** `/etc/systemd/system/webcheck.timer`

```ini
[Unit]
Description=Run Web Check every 10 minutes via Systemd Timer
Requires=webcheck.service

[Timer]
OnBootSec=1min
OnUnitActiveSec=10min
Persistent=true

[Install]
WantedBy=timers.target
```

*(Скриншот 6: Содержимое webcheck.timer)*
[ВСТАВЬТЕ СКРИНШОТ 6 ЗДЕСЬ]

---

### 2.3 Активация и проверка таймера

**Активация таймера:**
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now webcheck.timer
```

**Просмотр активных таймеров:**
```bash
sudo systemctl list-timers --all | grep -E "(webcheck|NEXT|LAST|UNIT)"
```

*(Скриншот 7: Список активных таймеров systemd)*
[ВСТАВЬТЕ СКРИНШОТ 7 ЗДЕСЬ]

---

### 2.4 Логирование через Journald
**Просмотр логов:**
```bash
sudo journalctl -u webcheck.service -n 5 --no-pager
```

*(Скриншот 8: Логи systemd в journalctl)*
[ВСТАВЬТЕ СКРИНШОТ 8 ЗДЕСЬ]

---

## 📊 Сравнительная таблица Cron vs Systemd

| Критерий | Cron | Systemd Timer |
|----------|------|---------------|
| **Синтаксис** | `*/5 * * * *` | `OnUnitActiveSec=10min` |
| **Логирование** | Файл логов (~/cron_requests.log) | Через journald (`journalctl -u service`) |
| **Зависимости** | Нет контроля зависимостей | Есть контроль (After, Requires) |
| **Пропущенные запуски** | Пропускаются без компенсации | `Persistent=true` компенсирует пропуски |
| **Управление** | `crontab -e`, `crontab -l` | `systemctl`, `journalctl` |
| **Интеграция** | Отдельный демон | Интегрирован в systemd экосистему |
| **Сложность** | Простой | Более сложный, но мощный |

---

## 🎯 Выполненные задачи

### ✅ Реализовано через Cron:
1. Создан bash-скрипт для отправки HTTP-запросов
2. Настроено расписание выполнения каждые 5 минут
3. Настроено логирование в файл
4. Проверена работоспособность

### ✅ Реализовано через Systemd:
1. Создан systemd service unit
2. Создан systemd timer unit с интервалом 10 минут
3. Настроена интеграция с journald для логирования
4. Активирован таймер с автозапуском

---

## 📈 Результаты и выводы

### Преимущества Cron:
1. **Простота** - интуитивно понятный синтаксис
2. **Универсальность** - работает на всех Unix-системах
3. **Минимальная конфигурация** - только одна строка в crontab

### Преимущества Systemd Timer:
1. **Интеграция** - единая система управления сервисами
2. **Контроль зависимостей** - можно указывать зависимости между сервисами
3. **Надёжность** - компенсация пропущенных запусков
4. **Централизованное логирование** - через journald

### Рекомендации:
- **Для простых задач** - использовать Cron
- **Для сложных систем с зависимостями** - использовать Systemd Timer
- **Для новых систем** - предпочтительнее Systemd как более современное решение

---

## 🛠 Использованные команды (шпаргалка)

```bash
# Cron
crontab -e                    # Редактировать crontab
crontab -l                    # Показать crontab
tail -f ~/cron_requests.log   # Мониторить логи в реальном времени

# Systemd
sudo systemctl daemon-reload                 # Перезагрузить конфигурацию
sudo systemctl enable --now webcheck.timer   # Включить и запустить таймер
sudo systemctl list-timers --all             # Показать все таймеры
sudo journalctl -u webcheck.service -f       # Мониторить логи сервиса
```

---

## 📚 Источники
1. Официальная документация Cron: `man 5 crontab`
2. Официальная документация Systemd: `man systemd.timer`
3. Ubuntu Manpages: https://manpages.ubuntu.com/

---

## ✍️ Подпись студента
_________________________
[ФИО студента]

*Дата: 13.12.2025*
